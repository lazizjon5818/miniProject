    ...
    <header className='border-b-2 bg-white shadow-md'>
          ...
          <ul className='flex items-center gap-10'>
            {links.map((link) => (
              <li key={link.id} className='hover:text-gray-800'>
                <button onClick={() => handleNavigate(link.href)}>
      ...
      <div className={`fixed bottom-0 left-0 w-full bg-gradient-to-t from-black via-gray-900 to-transparent backdrop-blur-lg py-4 flex flex-col items-center gap-4 z-50 transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className='w-full flex justify-around items-center text-white text-lg'>
          ...
          <button onClick={() => handleNavigate('/cart')} className='flex flex-col items-center gap-1'>
            <MdOutlineShoppingCart className='text-2xl' />
            <span>Cart</span>
          </button>
          <button onClick={() => handleNavigate('/wishlist')} className='flex flex-col items-center gap-1'>
            <CiHeart className='text-2xl' />
            <span>Wishlist</span>
          </button>
          <button onClick={() => handleNavigate('/account')} className='flex flex-col items-center gap-1'>
        ...
        <div className='w-full flex justify-around mt-4 text-white text-lg'>
          <button onClick={() => handleNavigate('/about')} className='flex flex-col items-center gap-1'>
            <BiInfoCircle className='text-2xl' />
            <span>About</span>
          </button>
          <button onClick={() => handleNavigate('/contact')} className='flex flex-col items-center gap-1'>
